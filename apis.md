---
name: Streadata.io API Gallery - AWS Simple Queue Service
description: This is a repository of aws simple queue service APIs, self-contained
  as a Github repository, with HTML and YAML representations of the APIs and their
  surrounding operations.
image: http://template.streamdata.gallery/assets/aws-simple-queue-service.png
created: "2018-02-24"
modified: "2018-05-24"
url: http://aws-simple-queue-service.streamdata.gallery/apis.json
specificationVersion: "0.14"
apis: []
tags: AWS Simple Queue Service
include:
- name: AWS Simple Queue Service
  url: https://raw.githubusercontent.com/streamdata-gallery-organizations/aws-simple-queue-service/master/_listings/aws-simple-queue-service/apis.yaml
maintainers:
- FN: Kin Lane
  x-twitter: apievangelist
  email: info@apievangelist.com
---